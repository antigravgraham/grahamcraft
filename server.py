...
def handle_connect():
    ...
    emit('player_joined', {"player_id": player_id, "player": game_state["players"][player_id]}, broadcast=True)
...
def handle_world_load(data):
    ...
    try:
        ...
        emit('world_loaded', {"filename": filename, "success": True, "game_state": game_state}, broadcast=True)
    except Exception as e:
...
def handle_gravity_toggle():
    ...
    emit('gravity_changed', {"has_gravity": game_state["world_settings"]["has_gravity"]}, broadcast=True)
